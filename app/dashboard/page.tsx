...
export default function Dashboard() {
  ...
  const chartW = 500, chartH = 180, padX = 10, padY = 10;
  const toPoint = (values: number[]) => values.map((v, i) => {
    const x = padX + (i / (values.length - 1)) * (chartW - 2 * padX);
    const y = padY + (1 - v / maxVisitor) * (chartH - 2 * padY);
    return `${x},${y}`;
  }).join(' ');
  ...
  // Generate pie chart SVG arcs
  const pieRadius = 80, pieInner = 50, pieCx = 100, pieCy = 100;
  const pieArcs = (() => {
    let cumAngle = -90;
    return purposeData.map((d) => {
      ...
      const x1i = pieCx + pieInner * Math.cos(endRad);
      const y1i = pieCy + pieInner * Math.sin(endRad);
      const x2i = pieCx + pieInner * Math.cos(startRad);
      const y2i = pieCy + pieInner * Math.sin(startRad);
      const path = `M ${x1o} ${y1o} A ${pieRadius} ${pieRadius} 0 ${largeArc} 1 ${x2o} ${y2o} L ${x1i} ${y1i} A ${pieInner} ${pieInner} 0 ${largeArc} 0 ${x2i} ${y2i} Z`;
      return { ...d, path };
    });
  })();
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 ml-0 dynamic-ml p-4 md:p-8 pb-24 md:pb-8 overflow-y-auto w-full">
          ...
          <div className="flex flex-col gap-6 max-w-7xl mx-auto">
            ...
            {/* Row 2: Visitor Trend + Visit Purpose */}
            <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-4">
              
              {/* Visitor Trend — Multi-line Chart */}
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                ...
                <div className="w-full mt-4" style={{ aspectRatio: `${chartW}/${chartH + 30}` }}>
                  <svg viewBox={`0 0 ${chartW} ${chartH + 30}`} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
                    {/* Grid lines */}
                    {[0, 0.25, 0.5, 0.75, 1].map((frac, i) => {
                      const y = padY + frac * (chartH - 2 * padY);
                      return <line key={i} x1={padX} y1={y} x2={chartW - padX} y2={y} stroke="#e2e8f0" strokeWidth="0.5" strokeDasharray="4 2" />;
                    })}
                    {/* Y-axis labels */}
                    {[0, 0.25, 0.5, 0.75, 1].map((frac, i) => {
                      const y = padY + frac * (chartH - 2 * padY);
                      const val = Math.round(maxVisitor * (1 - frac));
                      return <text key={i} x={padX - 2} y={y + 3} textAnchor="end" className="fill-slate-400" style={{ fontSize: '10px' }}>{val}</text>;
                    })}
                    {/* Yesterday line */}
                    ...
                    {/* Area fill */}
                    <polygon
                      points={`${padX},${chartH - padY} ${todayLine} ${chartW - padX},${chartH - padY}`}
                      fill="url(#todayGrad)"
                    />
                    ...
                    {todayVisitors.map((v, i) => {
                      const x = padX + (i / (todayVisitors.length - 1)) * (chartW - 2 * padX);
                      const y = padY + (1 - v / maxVisitor) * (chartH - 2 * padY);
                      return <circle key={i} cx={x} cy={y} r="3" fill="#3b82f6" stroke="white" strokeWidth="1.5" />;
                    })}
                    {/* X-axis labels */}
                    {hours.map((h, i) => {
                      const x = padX + (i / (hours.length - 1)) * (chartW - 2 * padX);
                      return <text key={i} x={x} y={chartH + 16} textAnchor="middle" className="fill-slate-400" style={{ fontSize: '10px' }}>{h}</text>;
                    })}
                  </svg>
                </div>
              </div>

              {/* Visit Purpose — Pie/Donut Chart */}
